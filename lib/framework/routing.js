...
const CONFIG = require(MAIN_DIR + "/package.json");
...
class router{
    ...
    async start(){
        ...
        ROUTER_FILE = require(MAIN_DIR + CONFIG.main);
        this.routes = ROUTER_FILE.routes;
        //add the public variables (if there are any)
        if(ROUTER_FILE.public) publicList = ROUTER_FILE.public;
...
module.exports = {
     ...
     */
    render(route, ...params){
    let main = ROUTER_FILE.routes[route];
